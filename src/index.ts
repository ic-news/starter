...
async function startAgent(character: Character, directClient: DirectClient) {
  try {
    ...
    // start news service
    try {
      elizaLogger.success("News service started successfully");
    } catch (error) {
      elizaLogger.error("Failed to start news service:", error);
    }
...
const startAgents = async () => {
  ...
  let serverPort = parseInt(settings.SERVER_PORT || "3084");
  ...
  if (serverPort !== parseInt(settings.SERVER_PORT || "3084")) {
